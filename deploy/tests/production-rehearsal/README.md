...
`q`×40 and `r`×40, as in `backend/tools/qa_server.py`), the port is bound to
...
1. The document carries exactly the CSP in `deploy/Caddyfile`, plus HSTS,
   `X-Frame-Options`, `nosniff`.
2. Sign-in over HTTPS sets an httpOnly, Secure, SameSite=Strict cookie.
3. Home, Team, Memory, Goals, Account, Privacy and a chat render with images,
   self-hosted fonts and stylesheets loaded and no CSP violation.
...
14. Readiness through Caddy reports production and a current schema.
15. At 390px: no horizontal overflow; the notice and the recovery button fit.
16. No CSP violations and no uncaught page errors over the whole run.
